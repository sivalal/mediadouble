...
s.successfulSearchEvent 		= 'event3';
//s.nullSearchEvent 				= 'event2';
s.searchTermVariable		    = 'eVar3';
//s.visitorId= s.gfid();

function s_doPlugins(s) {

	//s.prop74="11/21/2013 - V26.1";
    ...
    if(!s.eVar36)
    	s.eVar36=s.getQueryParam('IRID');

        /* s.clickThruQuality('CMP','event10','event11');
        s.eVar35 = s.crossVisitParticipation(s.campaign,'s_cmp','30','5','>','purchase',1);
...
/*
	...
	*/
	...
	//append linkTracking variables
	//if(!s.linkName && s.eVar2)s.linkName = s.eVar2;
	if(!s.inList("prop41",s.linkTrackVars))s.linkTrackVars=s.apl(s.linkTrackVars,"prop41",',',2);
	if(!s.inList("prop42",s.linkTrackVars))s.linkTrackVars=s.apl(s.linkTrackVars,"prop42",',',2);
	if(!s.inList("prop43",s.linkTrackVars))s.linkTrackVars=s.apl(s.linkTrackVars,"prop43",',',2);
	if(!s.inList("prop44",s.linkTrackVars))s.linkTrackVars=s.apl(s.linkTrackVars,"prop44",',',2);
	...
	// var tpA = s.getTimeParting('n','-8');
	//var tpMinute = tpA[0]; // Time at minute level
	//var tpDay = tpA[2]; // Day of week
	//s.eVar55 = tpMinute + " - " + tpDay;
	//if (s.eVar55)
	//		s.prop55="D=v55";
	...
	/*Get previous page, percent viewed, and viewed on initial load */
	//var ppvArray = s.getPercentPageViewed(s.pageName)
	//s.prop72=ppvArray[0]+ '||' + ppvArray[1] + '||' + ppvArray[2]

	//s.detectRIA('s_ria','eVar64','','13','1','');

	/*First page of Visit
	if(s.getVisitStart("s_visit")==1)s.events=s.apl(s.events,"event99",',',2); */

	/*get Visit Number*/
	/*s.prop68=s.getVisitNum(30);
	s.prop69=s.getVisitNum(90);

	if (s.channel)s.eVar9="D=ch";
	if (s.eVar10) s.prop7="D=v10";
	if (s.eVar11) s.prop8="D=v11";
	if (s.eVar53) s.prop52="D=v53";
	if (s.eVar57) s.prop57="D=v57";
	if (s.eVar58) s.prop58="D=v58";
	if (s.eVar63) s.prop63="D=v63";
	if (s.pageName) s.eVar49="D=pageName"; */
	if (s.campaign) s.prop29="D=v0";
	//s.prop26="D=g";
