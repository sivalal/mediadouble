 ...
 */
mdlDirectTvApp.controller('signupPaymentErrorCtrl', ['$scope', '$modal', 'dateFilter', '$http', 'SocialService', '$rootScope', '$sce', 'Sessions', 'Authentication', 'configuration', '$filter', '$window', 'Vindicia', '$location', 'analyticsService', '$cookieStore', '$log', '$routeParams', 'Gigya',
    function($scope, $modal, dateFilter, $http, SocialService, $rootScope, $sce, Sessions, Authentication, configuration, $filter, $window, Vindicia, $location, analyticsService, $cookieStore, $log, $routeParams, Gigya) {
        ...
        if (!!authID && !!paymentMethodBasedSessionID && (authID == paymentMethodBasedSessionID) && ($scope.routePath == '/signupError')) {
