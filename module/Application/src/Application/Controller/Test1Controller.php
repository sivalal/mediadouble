...
class SearchController extends AbstractActionController
{
    ...
    {   
        ...
        $assetTypeArray = array();
        if(isset($assetType) && !empty($assetType)){
            $splitAssestTypes =  json_decode($assetType, true);
            $assetTypeArray   = $splitAssestTypes['types'];
        }
        $response            = $this->getResponseWithHeader(); 
        $externalApiAdapter  = $this->getServiceLocator()->get('externalApi');
        $config              = $this->getServiceLocator()->get('Config');
        $route               = $config['general_routes']['movieAndTv']['most_popular_new'];
        $param['score_type'] ='top_titles';
        $urlWithSignature    = $this->generateOoyalaApiUrlWithSignature('GET', $route, $param);
        $mostPopularResult   = $externalApiAdapter->callHttpUtilityService($urlWithSignature);


        if($mostPopularResult['responseStatus'] == 200) {
            /**
             * Success Logger
             */
            $Logger       = $this->getServiceLocator()->get('Zend\Log\Logger');
            $Logger->log(\Zend\Log\Logger::INFO, 'Success on most popular call');
            if(!empty($mostPopularResult['responseContent'])) {
                $titleIdArray = $externalApiAdapter->getEmbedcodeList($mostPopularResult['responseContent']['results']);
                if(!empty($titleIdArray)) {
                    foreach ($titleIdArray as $key => $titleId) {
                        $queryOption    = array('q' => '{"id":"'.$titleId.'"}');
