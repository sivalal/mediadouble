...
class Block
{
    ...
    /**
     * maintain a list of key ids.
     * list of keys and id Important
     * params  $key string
     * 
     * return id long   ID of the shared memory block you want to manipulate
     */
    public function keyId($key) {
        $keyIdArr=array(
            'Sample'=>2,
            'appgrid_metadata'=>222
        );
        if(!empty($keyIdArr[$key])){
            return $keyIdArr[$key];
        } else {
            print_r("Invalid key string for shmop, please define new key string  in KeyidArr");
            die();
        }
    }
